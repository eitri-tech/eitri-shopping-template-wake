...
export default function ProductCard(props) {
	...
	const buyItem = async (variationId, value) => {
		...
		try {
			...
			if (_product && _product.productVariantId) {
				...
				// Fallback: usar o produto original se não conseguir buscar com variações
				const cart = await addItemToCart(product.productVariantId, 1)
				setClickedBuyLink(false)
			}
		} catch (e) {
			// Fallback em caso de erro: tentar adicionar o produto original
			try {
				await addItemToCart(product.productVariantId, 1)
				setClickedBuyLink(false)
			} catch (fallbackError) {}
		}
		setLoadingVariations(false)
	...
	return (
		<View className={`relative flex flex-col snap-start flex-shrink-0 ${className ? className : 'w-full'}`}>
			{showFavoriteIcon && (
				<FavoriteButton
					className={
						'absolute top-2 right-2 z-10 flex items-center justify-center w-8 h-8 rounded-full bg-white/40 backdrop-blur-sm'
					}
			...
			<View className='w-full h-full flex flex-col gap-1'>
				{onlyImage ? (
					...
					<View className='relative flex flex-col h-full'>
						<View
							className={`relative w-full grow-1 ${disableImageScroll ? 'overflow-hidden' : 'overflow-x-auto snap-x snap-mandatory scroll-smooth'} flex flex-row`}
							onClick={() => goToProduct(product)}>
							{' '}
							{images?.length > 0 &&
				...
				{showProductInfo && (
					<View className={`w-full flex flex-col gap-1`}>
						{/* Nome do produto */}
						<Text className={`line-clamp-1`}>{product.productName}</Text>

						{/* Preços */}
						<View className='flex flex-col gap-1'>
							{/* Preço com desconto (se houver) ou preço normal */}
							<Text className={`font-bold text-gray-900`}>{formatCurrency(product.prices.price)}</Text>

							{/*/!* Preço original riscado (só aparece se houver desconto) *!/*/}
							{/*{getDiscountPercentage() && (*/}
							{/*	<Text*/}
							{/*		className={`${size === 'small' ? 'text-[8px]' : 'text-xs'} text-gray-500 line-through`}>*/}
